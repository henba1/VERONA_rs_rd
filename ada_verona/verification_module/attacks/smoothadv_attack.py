...
class SmoothAdvAttack(Attack):
    """
    PGD (L2 or L_inf) with SMOOTHADV (Salman et al.) loss.

    This is a minimal extension of `PGDAttack`:
    - Still performs standard PGD projection + optional random start.
    - Uses SMOOTHADV (Salman et al.) plug-in estimator:
        loss = -log( (1/m) * sum_i p_y(x_adv)_i ), where each forward draws fresh randomness.
    ...
    def __init__(
        ...
        *,
        ...
        self.name = (
            f"SmoothAdvAttack (iterations={self.number_iterations}, eot_samples={self.eot_samples}, "
    ...
    def execute(self, model: Module, data: Tensor, target: Tensor, epsilon: float) -> Tensor:
        ...
        try:
            for _ in range(self.number_iterations):
                ...
                # SMOOTHADV (Salman et al.) plug-in estimator for Eq. (6):
                #   loss = -log( (1/m) * sum_i p_y(x + delta_i) )
                # where each forward pass draws fresh randomness.
                #
                # Numerically stable - using logsumexp over log-probs of the target class.
                m = self.eot_samples
                log_p_y_samples = []
                for _ in range(m):
                    logits = model(adv_images)  # [B, C], stochastic draw each time
                    log_probs = torch.log_softmax(logits, dim=1)  # stable
                    # gather log p_y for each batch element
                    log_p_y = log_probs.gather(1, target.view(-1, 1)).squeeze(1)  # [B]
                    log_p_y_samples.append(log_p_y)
                # Stack: [m, B]
                log_p_y_samples = torch.stack(log_p_y_samples, dim=0)
                # log(mean_i p_y_i) = logsumexp(log p_y_i) - log(m)
                log_mean_p_y = torch.logsumexp(log_p_y_samples, dim=0) - math.log(m)  # [B]
                # SMOOTHADV CE on the smoothed soft classifier: -log E[p_y]
                loss = (-log_mean_p_y).mean()
