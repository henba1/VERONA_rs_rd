...
def main():
    ...
    sample_size = 300
    random_seed = 42
    use_first_n = True  # Use first N instances in order; if False, use random/stratified sampling
    experiment_tag = None
    ...
    # network_folder used for ONNX/file-based models; HuggingFace models are loaded separately
    ...
    if use_huggingface_models:
        ...
        network_list = load_huggingface_classifier_networks(
            model_ids=huggingface_model_ids,
    ...
    if len(network_list) == 0:
        logging.error("No models loaded. Check use_huggingface_models and huggingface_model_ids, or MODELS_DIR.")
        return
    ...
    # For HuggingFace ViT: 224x224, no flatten (model expects spatial input)
    # For ONNX/pytorch: 32x32, flatten=True (standard CIFAR-10)
    image_size = (224, 224) if use_huggingface_models else None
    flatten = not use_huggingface_models

    if use_first_n:
        cifar10_torch_dataset, original_indices = get_first_n(
            dataset_name=dataset_name,
    ...
    # TODO: currently not the most efficient, as we run inference twice
